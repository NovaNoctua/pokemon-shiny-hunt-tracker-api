...
export default class extends BaseSeeder {
  async run() {
    ...
    for (const group of data.results) {
        ...
        const generationId = parseInt(groupDetails.generation.name.split('-')[1]) || 0 // generation-iii -> 3 (Wait, API uses 'i', 'ii', 'iii')...
        ...
        const genId = parseInt(groupDetails.generation.url.split('/').filter((s:any) => s).pop())
